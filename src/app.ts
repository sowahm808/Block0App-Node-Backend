...
export async function buildApp(overrides?: any) {
  ...
  const fb = overrides?.firebase ?? initializeFirebase();
  const users = overrides?.users ?? new UsersRepository(fb.db, env.FIREBASE_USERS_COLLECTION);
  const sessions =
    overrides?.sessions ?? new AuthRepository(fb.db, env.FIREBASE_REFRESH_SESSIONS_COLLECTION);
  const authService = overrides?.authService ?? new AuthService(fb.auth, users, sessions, env);
  const readiness = overrides?.readiness ?? new ReadinessService(fb.db);
  const meta = {
    name: 'MindUnlocking API',
    version: 'v1',
    status: 'available',
    links: { health: '/health/live', readiness: '/health/ready', auth: '/api/v1/auth' },
  };
