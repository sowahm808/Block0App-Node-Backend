...
4. **Save** sends either the complete edited `LearningPackImportPayload` or `{ draft: editedPayload }` to `PUT /api/v1/admin/learning-packs/imports/:importId/draft`. The wrapped form lets the review screen send the `draft` property returned by the detail endpoint without reshaping it. Never send the original file again. The backend preserves `sourceFileName` and revalidates.
